...
public class DigitalClockSample extends Application {
    ...
    public void start(Stage stage) throws IOException {
        ...
        //creat another label under the label and set the format to suit the value that 
        Label tLabel = new Label("Timer label");
        DigitalClock dc = new DigitalClock();
        
        VBox vb =new VBox();
        vb.getChildren().addAll(tLabel, dc);
...
class DigitalClock extends Label {
    ...
    private void bindToTime2() {
        //DateTimeFormatter SHORT_TIME_FORMATTER;
        DateTimeFormatter SHORT_TIME_FORMATTER = DateTimeFormatter.ofPattern("HH:mm:ss");

        Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(0),
                //event -> setText(LocalTime.now().format(SHORT_TIME_FORMATTER))), //try n put another way to take time 
                event -> setText("blah blah blah")), //try n put another way to take time 
                new KeyFrame(Duration.seconds(1)));
